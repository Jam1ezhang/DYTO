...
class LlavaMetaForCausalLM(ABC):
    ...
    def kmeans_cluster_exp(self,image_features,tw_finch):
        from sklearn.cluster import KMeans
        
        # Extract global features and convert to numpy array
        image = image_features[:,0,:].detach().cpu().numpy() #[num_frame,577,4096]
        
        # Use K-means clustering with K=9
        kmeans = KMeans(n_clusters=6, random_state=42)
        classification = kmeans.fit_predict(image)
        ...
        # For storing new encodings
        new_embeddings = []
        selected_indexs = []
        
        # Select images according to class classification
        if len(unique_classes) <= 6:
            num = 12 // len(unique_classes)
            for class_label in unique_classes:
                # Get image indices for current classification
                class_indices = (classification == class_label).nonzero(as_tuple=True)[0]
                
                # Take x images, if less than x then take all
            ...
            for class_label in unique_classes:
                ...
                # Select middle image from classification
        ...
        for index in selected_indexs:
            # Remove first patch from original encoding, only keep patch embeddings [1:577]
            embedding = image_features[index, 1:, :]  # dimension [576, 4096]
            # Add to new embeddings
            new_embeddings.append(embedding)

        # Connect all embeddings together to form new tensor
        new_image_features = torch.stack(new_embeddings)
    ...
    def finch_cluster(self,image_features,tw_finch):
        ...
        # Classification results of global_embedding, assumed to be obtained, dimension [100, 1]
        # Remove duplicate image indices in each class
        unique_classes = torch.unique(classification, return_inverse=False, return_counts=False)
        # For storing new encodings, dimension [122, 576, 4096]
        new_embeddings = []
        selected_indexs = [] 
        # Select images according to class classification
        if len(unique_classes) <= 6 : 
            ...
            for class_label in unique_classes:
                ...
                # Assume selecting the first image in the classification
                selected_index = class_indices[len(class_indices)//2 if len(class_indices) > 2 else -1]
                selected_indexs.append(selected_index)
    ...
    def temporal_aggregation(self, image_features, temporal_aggregation):
        ...
        if temporal_aggregation == "concat":
            ...
            if T<=12:
                ...
                r_merge_list=[288]
                merge_ratio = 288-(3680 // T)
                r_merge_list.append(merge_ratio if merge_ratio<=144 else 144)
            ## ToMe local merge
            video_features = []
            clus_image_features = list(torch.unbind(clus_image_features, dim=0)) #[T*(N,D)]
            for image in clus_image_features:
                image = merge_tokens(image.unsqueeze(0),r_merge_list=r_merge_list,clips=1,mode="amax")
                video_features.append(image.squeeze(0))
            video_features = torch.stack(video_features,dim=0)
            image_features = rearrange(video_features, 't n d -> (t n) d')
        else:
